...
export const MovieCard = ({upcoming}: {upcoming: MovieSummary}) => {
  return (
    <div className="w-[229.73px] h-109.75 bg-[#F4F4F5] rounded-lg overflow-hidden gap-1">
      <img className='w-full h-85 bg-cover bg-center' src={`https://image.tmdb.org/t/p/w500${upcoming.poster_path}`} alt="" />
      <div className='p-2'>
      <div className='flex gap-1'>
        ...
        <p className=' text-[#09090B] text-sm font-medium'>{upcoming.vote_average.toFixed(1)}/10</p>

      </div>
      <p className='text-[#09090B] font-normal text-lg'>{upcoming.title}</p>
      </div>
    </div>
  )
}
